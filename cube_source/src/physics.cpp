...
bool plcollide(dynent *d, dynent *o, float &headspace, float &hi, float &lo) // collide with player or monster
{
    ...
    if(fabs(o->o.x-d->o.x)<r && fabs(o->o.y-d->o.y)<r) 
    {
		...
		if (d->GetMonsterState()) return false; // hack
...
bool collide(dynent *d, bool spawn, float drop, float rise)
{
    ...
    float hi = 127, lo = -128;
    float minfloor = (d->GetMonsterState() && !spawn && d->health>100) ? d->o.z-d->eyeheight-4.5f : -1000.0f;  // big monsters are afraid of heights, unless angry :)
...
void physicsframe()          // optimally schedule physics frames inside the graphics frames
{
    ...
    {
        int faketime = (int)curtime+physicsfraction;
...
void moveplayer(dynent *pl, int moveres, bool local, float curtime)
{
    ...
    else                        // apply velocity with collision
    {
        if(pl->onfloor || water)
        {
            ...
            else if(pl->timeinair>800)   // landing after a big jump
            {
    ...
    if (!pl->inwater && water) 
	{ 
		snd_event( ( pl == player1 ) ? AK::EVENTS::SPLASHIN_PLAYER : AK::EVENTS::SPLASHIN_MONSTER, pl); 
		pl->vel.z = 0; 
	}
    else if(pl->inwater && !water) 
	{
		snd_event( ( pl == player1 ) ? AK::EVENTS::SPLASHOUT_PLAYER : AK::EVENTS::SPLASHOUT_MONSTER, pl); 
	}
...
void moveplayer(dynent *pl, int moveres, bool local)
{
    loopi(physicsrepeat) 
	{
		moveplayer(pl, moveres, local, i ? curtime/physicsrepeat : curtime-curtime/physicsrepeat*(physicsrepeat-1));
	}
};
